...
| `stem-button-panel-tester` | A | v1.3.2 | 2026-05-07 | bumped to v1.3.2 via the seed-then-restore recipe (#42 / #45); v1.3.3 / v1.4.0 / v1.5.0 bump pending — v1.5.0 brings the GUI standards trio that pairs with the FuncUI migration |
| `stem-dictionaries-manager` | A | v1.3.2 | 2026-05-07 | adopted at v1.3.2; app with internal class library — see MIGRATION.md; v1.3.3 / v1.4.0 / v1.5.0 bump pending — v1.5.0 brings the GUI standards trio that pairs with the FuncUI migration |
...
## Pending adoption — v1.5.0

`v1.5.0` introduces the **GUI standards trio** for archetype A: `GUI` (Avalonia + FuncUI + Elmish-MVU shape, `<App>.GUI/` layout, manual-DI composition root, pure-`Update` invariant, three-layer test posture), `DESIGN_SYSTEM` (Fluent theme + light default, 4-pt spacing scale, Fluent System Icons, Poppins typography, Stem brand palette + division identity colors, F# strings-module i18n, four error-and-progress surfaces), and `APP_SHELL` (canonical view catalogue — Settings / About / LanguagePicker / NotificationCenter / ConnectionStatus — wired through a typed `ShellSlots<'msg>` record; Navigation pinned to a left sidebar). Together they prescribe the target state for the in-flight WinForms/WPF → Avalonia migrations and ship the Poppins font bundle each adopted GUI project will embed. The rollout script gains placeholder substitution in destination path segments (so `archetypes/A/src/{{App}}.GUI/Resources/fonts/Poppins-*.ttf` lands at the substituted per-repo path) plus binary-safe handling for `.ttf` / `.otf` / `.woff` / `.woff2` extensions.

Per-repo adoption follows `MIGRATION.md`'s minor-bump procedure — re-run `eng/apply-repo-standard.ps1 -StandardVersion v1.5.0`, review the diff (three new `docs/Standards/*.md` files, `Resources/fonts/` bundle landed in the per-app GUI project, `Directory.Packages.props` template-version stamp), bump the per-repo `CLAUDE.md`, then update the table above. Adopters still on a pre-v1.5.0 GUI stack also need to wire the canonical components (`LanguagePicker`, `NotificationCenter`, plus `ConnectionStatus` when `Capabilities.connection = Connected`) into the top-level `View.fs` as a `ShellSlots<Msg>`. The `<App>.GUI.Windows` legacy carve-out keeps in-flight WinForms/WPF code compliant while the migration proceeds.

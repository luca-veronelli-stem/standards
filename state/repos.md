...
`v1.4.0` migrates workflow templates from full inline jobs to thin caller stubs that delegate to reusable workflows shipped from this repo (`dotnet-ci.yml`, `mirror-bitbucket.yml`, `release-archetype-{a,b}.yml`). After the bump, a pin change in a reusable workflow (e.g. `actions/setup-dotnet`) propagates to adopted repos on their next CI run without a per-repo PR. See `MIGRATION.md`'s "Rollout phase for v1.4.0" for the one-time Actions-access prerequisite (the standards repo is private, so cross-repo reusable-workflow calls need `access_level: user` flipped once on the workflow-source repo). v1.4.0 also folds in the v1.3.3 template refreshes (GHA pin updates from issue #49, NuGet bumps from issue #51, `.github/dependabot.yml` on `standards` itself from issue #50) and fixes the `xunit 2.9.4` pin regression (#64).
